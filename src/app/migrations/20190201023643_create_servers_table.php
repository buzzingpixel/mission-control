...
class CreateServersTable extends AbstractMigration
{
    public function change() : void
    {
        $this->table('servers', [
            ...
            ->addColumn('address', 'string', ['comment' => 'The server address or IP'])
            ->addColumn('ssh_port', 'string', ['comment' => 'The server ssh port'])
            ->addColumn('ssh_key_guid', 'binary', [
                'null' => true,
                'limit' => 16,
                'comment' => 'The SSH key to use when connecting to the server',
            ])
            ->addColumn('ssh_user_name', 'string', ['comment' => 'The server ssh user name'])
            ->create();
    }
}
